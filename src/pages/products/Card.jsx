...
function Card({ item }) {
  ...
  const handleSubmit = () => {
    const data = {
      fullName,email, amount: bidAmount
    }
    console.log(data)
    toggleModal();
    ...
    <>
      <div className="flex group flex-col space-y-10 rounded-lg overflow-hidden border border-slate-400/10 pb-8 hover:shadow-xl duration-500 ease-in-out hover:shadow-white/5 relative">
        ...
        <div className="flex flex-col items-start relative">
          ...
          {sale && (
            <div className="flex space-x-2 items-center justify-center px-4 py-1 bg-gradient-to-r from-red-400 via bg-fuchsia-500 to-indigo-500 rounded-xl absolute bottom-5 left-5">
